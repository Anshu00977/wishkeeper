(function () {
  ...
  fetch(proxyUrl + "/api/products?shop=" + encodeURIComponent(shop) + "&customerId=" + encodeURIComponent(customerId))
    ...
    .catch(function () {
      document.getElementById("wl-grid").innerHTML =
        '<div class="wl-empty"><p>Could not load your wishlist. Please try again.</p></div>';
    });
  ...
  function money(a, c) {
    return new Intl.NumberFormat("en-US", { style: "currency", currency: c || "USD" }).format(parseFloat(a));
  }
  ...
  window.__wlRemove = function (pid, vid, fromCart) {
    ...
    fetch(proxyUrl + "/api/wishlist", {
      ...
      if (!fromCart) showToast(title, "Removed from Wishlist", imgSrc);
      ...
      fetch(proxyUrl + "/api/wishlist?shop=" + encodeURIComponent(shop) + "&customerId=" + encodeURIComponent(customerId))
        ...
        .then(function (data) {
          ...
          try { localStorage.setItem("wl_count_" + customerId, count); } catch (e) { }
