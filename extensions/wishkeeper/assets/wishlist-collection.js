  ...
  function findProductCards() {
    ...
    for (var i = 0; i < linkSelectors.length; i++) {
      ...
      for (var j = 0; j < links.length; j++) {
        ...
        var handle = match[1];
        var container = link.closest('.product-card-wrapper, .card-wrapper, .grid-product, .product-item, .collection-product-card, .product-card, .card, .grid__item');
        ...
        var imgWrap = container.querySelector('.card__media, .card__inner, .product-card__image-wrapper, .card-media, .grid-product__image-wrap, .media, .card__image-wrapper');
