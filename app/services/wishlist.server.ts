...
export async function addWishlistItem(
  ...
  // Match by product only (not variant) so the same product added from
  // different entry points (e.g. collection grid vs. product page, which
  // may or may not know the selected variant) is treated as one wishlist
  // item, not duplicated.
...
export async function removeWishlistItem(
  ...
  // Remove by product only (not variant) to match the add logic above.
